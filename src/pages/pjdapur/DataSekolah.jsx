...
function DataSekolah() {
  ...
  const handleSaveData = (e) => {
    ...
    const npsnSudahAda = dataSekolah.some(
      (item) => item.npsn === formData.npsn && item.id !== formData.id,
    );
    ...
    if (viewMode === "create") {
      const dataBaru = [...dataSekolah, payload];
      setDataSekolah(dataBaru);
    } else {
      const dataBaru = dataSekolah.map((item) =>
                    ...
                    dataSekolah.map((item, index) => (
                      ...
                      >
                        ...
                        <input
                          ...
                          value={formData.npsn}
                          onChange={(e) => {
                            const angka = e.target.value.replace(/\D/g, ""); // hanya angka
                            setFormData({
                              ...formData,
                              npsn: angka,
                            });
                          ...
                          className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-700 rounded-xl"
                        />
