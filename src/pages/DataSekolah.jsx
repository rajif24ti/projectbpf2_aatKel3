...
function DataSekolah() {
  ...
  const [dataSekolah, setDataSekolah] = useState([
    { id: 1, npsn: '20103211', nama: 'SDN 01 Menteng', akreditasi: 'A', jumlahSiswa: 340, alamat: 'Jl. Pemuda No. 10, Jakarta Pusat' },
    { id: 2, npsn: '20104562', nama: 'SDN 05 Merdeka', akreditasi: 'A', jumlahSiswa: 210, alamat: 'Jl. Merdeka Barat No. 45, Jakarta Pusat' },
    { id: 3, npsn: '20108923', nama: 'SD Swasta Kartika', akreditasi: 'B', jumlahSiswa: 185, alamat: 'Jl. Jend. Sudirman Kav. 21, Jakarta Selatan' },
    { id: 4, npsn: '20107741', nama: 'SMPN 12 Jakarta', akreditasi: 'A', jumlahSiswa: 520, alamat: 'Jl. Wijaya Kebayoran Baru, Jakarta Selatan' },
  ]);
  ...
  const openCreateMode = () => {
    setFormData({ id: null, npsn: '', nama: '', akreditasi: 'A', jumlahSiswa: '', alamat: '' });
    setViewMode('create');
  };
  ...
  const handleSaveData = (e) => {
    ...
    if (viewMode === 'create') {
      ...
      setDataSekolah(dataSekolah.map((item) => (item.id === formData.id ? payload : item)));
    }
    setViewMode('index');
  };
  ...
  const handleDeleteData = (id, nama) => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus data "${nama}"? Tindakan ini tidak dapat dibatalkan.`)) {
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {viewMode === 'index' && 'Manajemen data sekolah penerima distribusi program makanan bergizi (MBG).'}
              {viewMode === 'create' && 'Registrasi profile dan kuota data sekolah mitra baru.'}
              {viewMode === 'edit' && 'Pembaruan data kuota siswa atau akreditasi sekolah.'}
            </p>
          </div>
                  ...
                  {dataSekolah.length === 0 ? (
                    ...
                    dataSekolah.map((item, index) => (
                      <tr key={item.id} className="text-gray-700 dark:text-gray-300 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition">
                        ...
                        <td className="p-4 font-mono text-xs font-semibold text-gray-600 dark:text-gray-400">{item.npsn}</td>
                        <td className="p-4 font-bold text-gray-800 dark:text-gray-100">{item.nama}</td>
                        <td className="p-4 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getBadgeAkreditasi(item.akreditasi)}`}>
                            {item.akreditasi}
                          </span>
                        </td>
                        <td className="p-4 text-center font-medium text-gray-800 dark:text-gray-200">
                          {item.jumlahSiswa.toLocaleString('id-ID')} <span className="text-xs text-gray-400 font-normal">Anak</span>
                        </td>
        ...
        {(viewMode === 'create' || viewMode === 'edit') && (
          <div className="max-w-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700/60 rounded-2xl p-6 transition-all duration-200">
            <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-6 flex items-center">
              ...
              {viewMode === 'create' ? 'Registrasi Sekolah Baru' : `Ubah Profil Sekolah: ${formData.nama}`}
            </h2>
            ...
            <form onSubmit={handleSaveData} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Field: NPSN */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">NPSN (Nomor Pokok Sekolah Nasional)</label>
                  <input
                    type="text"
                    name="npsn"
                    value={formData.npsn}
                ...
                {/* Field: Nama Sekolah */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Nama Instansi Sekolah</label>
                  <input
                    type="text"
                    name="nama"
                    value={formData.nama}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Field: Akreditasi */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Peringkat Akreditasi</label>
                  <select
                    name="akreditasi"
                ...
                {/* Field: Jumlah Siswa */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jumlah Siswa Aktif (Kuota Porsi)</label>
                  <input
                    type="number"
                    name="jumlahSiswa"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Alamat Lengkap Distribusi</label>
                <textarea
                  name="alamat"
              ...
              <div className="p-4 bg-amber-50/60 dark:bg-amber-500/5 border border-dashed border-amber-200 dark:border-amber-500/20 rounded-xl">
                <p className="text-xs text-amber-600 dark:text-amber-400 leading-relaxed">
                  ⚠️ <strong>Penting:</strong> Data <em>Jumlah Siswa Aktif</em> digunakan langsung oleh sistem produksi untuk menentukan kuota takaran bahan masakan porsi harian otomatis.
                </p>
              </div>
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 dark:border-gray-700/60">
                ...
                >
                  {viewMode === 'create' ? 'Simpan Sekolah' : 'Simpan Perubahan'}
                </button>
              </div>
            </form>
          </div>
