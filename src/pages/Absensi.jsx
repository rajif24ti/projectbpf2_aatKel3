...
function Absensi() {
  ...
  const [dataAbsensi, setDataAbsensi] = useState([
    { id: 1, nama: 'Siti Aminah', jamMasuk: '00:00', jamPulang: '07:00', status: 'Hadir' },
    { id: 2, nama: 'Budi Santoso', jamMasuk: '00:05', jamPulang: '07:00', status: 'Hadir' },
    { id: 3, nama: 'Rina Lestari', jamMasuk: '01:15', jamPulang: '07:00', status: 'Terlambat' },
    { id: 4, nama: 'Agus Pratama', jamMasuk: '00:45', jamPulang: '08:00', status: 'Terlambat' },
    { id: 5, nama: 'Dewi Kartika', jamMasuk: '-', jamPulang: '-', status: 'Izin' },
  ]);

  // BATAS TOLERANSI JAM MASUK
  const BATAS_JAM_MASUK = "00:15";
  ...
  const openCreateMode = () => {
    setFormData({ id: null, nama: '', jamMasuk: '', jamPulang: '', tipeStatus: 'Kerja' });
    setViewMode('create');
  };
  ...
  const openEditMode = (item) => {
    ...
    setFormData({
      ...
      jamPulang: item.jamPulang === '-' ? '' : item.jamPulang
    });
    setViewMode('edit');
  };
  ...
  const handleSaveData = (e) => {
    ...
    let finalJamPulang = formData.jamPulang;
    ...
    if (formData.tipeStatus === 'Kerja') {
      ...
      if (!timeRegex.test(finalJamMasuk) || !timeRegex.test(finalJamPulang)) {
        alert("Format Jam Tidak Valid! Gunakan format 24 jam standar (Contoh: 00:30, 07:00).");
        return; // Menghentikan fungsi simpan jika tidak valid
      }

      // Logika Otomatisasi Status berdasarkan jam masuk
      finalStatus = finalJamMasuk > BATAS_JAM_MASUK ? 'Terlambat' : 'Hadir';
    } else {
    ...
    if (viewMode === 'create') {
      ...
      setDataAbsensi(dataAbsensi.map((item) => (item.id === formData.id ? payload : item)));
    }
    setViewMode('index');
  };
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold tracking-tight">
              Absensi Tim Pengolahan
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {viewMode === 'index' && 'Rekap harian log data masuk kerja dini hari (00:00 - 07:00 / 08:00).'}
              {viewMode === 'create' && 'Formulir input jam masuk kerja operasional baru.'}
              ...
              <table className="table-auto w-full">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-700/40 text-gray-500 dark:text-gray-400 text-left text-xs uppercase font-bold tracking-wider border-b border-gray-100 dark:border-gray-700">
                    ...
                    <th className="p-4">Nama Pegawai Pengolahan</th>
                  ...
                  {dataAbsensi.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="p-8 text-center text-gray-400 dark:text-gray-500">
                        Tidak ada catatan absensi tim pengolahan hari ini.
                      </td>
                    </tr>
                  ) : (
                    dataAbsensi.map((item, index) => (
                      <tr key={item.id} className="text-gray-700 dark:text-gray-300 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition">
                        ...
                        <td className="p-4 font-mono text-xs">{item.jamMasuk}</td>
                        <td className="p-4 font-mono text-xs">{item.jamPulang}</td>
        ...
        {(viewMode === 'create' || viewMode === 'edit') && (
          <div className="max-w-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700/60 rounded-2xl p-6 transition-all duration-200">
            <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-6 flex items-center">
              ...
              {viewMode === 'create' ? 'Tambah Log Kehadiran Pengolahan' : `Ubah Data Absensi: ${formData.nama}`}
            </h2>

            <form onSubmit={handleSaveData} className="space-y-5">
              {/* Field: Nama Pegawai */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Nama Lengkap Pegawai</label>
                <input
                  type="text"
                  name="nama"
                  value={formData.nama}
                  ...
                  className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none transition text-sm text-gray-800 dark:text-gray-100 disabled:opacity-60"
                />
              </div>

              {/* Jenis Kategori Absensi */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Kategori Kehadiran</label>
                <select
                  name="tipeStatus"
                ...
                >
                  <option value="Kerja">Absen Kerja (Otomatis Hadir/Terlambat)</option>
                ...
                {formData.tipeStatus === 'Kerja' && (
                  <p className="text-xs text-amber-500 mt-1.5 font-medium">
                    * Wajib diisi dengan format waktu jam digital (contoh 00:00 atau 07:30).
                  </p>
                )}
              </div>

              {/* Baris Grid Jam Masuk & Pulang */}
              {formData.tipeStatus === 'Kerja' ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jam Masuk (Format HH:MM)</label>
                    <input
                      type="text"
                      name="jamMasuk"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jam Pulang (Format HH:MM)</label>
                    <input
                      type="text"
                      name="jamPulang"
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-700/20 border border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                  <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
                    Jam kerja dinonaktifkan karena pegawai berstatus <strong>{formData.tipeStatus}</strong>.
                  </p>
                </div>
              )}
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 dark:border-gray-700/60">
                ...
                >
                  {viewMode === 'create' ? 'Simpan Rekaman' : 'Simpan Perubahan'}
                </button>
              </div>
            </form>
          </div>
